...
function bignum_nonneg_quorem(bignum_a, bignum_b)
{
    ...
    if (len_b > 1)
    {
        ...
        // TODO: translate multi-digit-divisor-div function from Scheme code below
    }
    else
    {
...
function bignum_square(bignum)
{
    return bignum_mul(bignum, bignum);
}

function bignum_pow(bignum, n)
{
    // bignum^n

    if (n === 0)
        return bignum_from_js(1);
    else if (n === 1) // optimization
        return bignum;
    else if ((n & 1) === 0)
        return bignum_square(bignum_pow(bignum, n>>1));
    else
        return bignum_mul(bignum, bignum_square(bignum_pow(bignum, n>>1)));
}

function bignum_root(bignum, n)
{
    // floor(n-th root of bignum)

    var g = bignum_shift(bignum_from_js(1),
                         Math.floor((bignum_width(bignum)+n-1)/n));
    var old;

    do {
        var a = bignum_pow(g, n-1);
        var b = bignum_mul(a, bignum_from_js(n));
        var c = bignum_mul(a, bignum_from_js(n-1));
        old = g;
        g = bignum_div(bignum_add(bignum, bignum_mul(g, c)), b);
    } while (num_lt(g, old));
    return old;
}

function bignum_width(bignum)
{
    // Number of significant bits in a bignum not counting sign bit(s)

    if (!bignum_nonneg(bignum))
        bignum = bignum_not(bignum);
    ...
    return width + (len-1) * bignum_radix_log2;
}
...
function num_square(a) // a is a JS integer or bignum
{
    return num_to_js(bignum_square(num_from_js(a)));
}

function num_pow(a, n) // a is a JS integer or bignum, n is a JS integer
{
    return num_to_js(bignum_pow(num_from_js(a), n));
}

function num_root(a, n) // a is a JS integer or bignum, n is a JS integer
{
    return num_to_js(bignum_root(num_from_js(a), n));
}

function num_width(a) // a is a JS integer or bignum
{
    return bignum_width(num_from_js(a));
}
...
function num_to_string(a, radix) // a is a JS integer or bignum, radix is a JS integer
{
    ...
    else if (radix === undefined || radix === 10)
        return a.toString();
    else
